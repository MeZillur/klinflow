...
/* ============================================================================
 | 12b) Global "My Profile" (slugless) — /me
 *===========================================================================*/
// This uses session tenant_user + tenant_org; no slug needed.
if (strpos($path, '/me') === 0) {
    $MC = new MeController();
    ...
    // unknown /me subroute → 404
    friendly404('Unknown profile route.', '/');
}
...
if (preg_match('#^/t/([A-Za-z0-9_-]+)(?:/(.*))?$#', $path, $m)) {
    ...
    if ($seg1 === 'users') {
        ...
        // /t/{slug}/users/me (legacy) -> redirect to global /me
        if ($seg12 === 'users/me') {
    ...
    // /t/{slug} → default module if enabled, otherwise /dashboard
    if ($tail === '' && $method === 'GET') {
        try {
            $def = \App\Services\ModuleAccess::defaultModuleFor($slug);
        } catch (\Throwable $e) {
            $def = null;
        }

        if ($def) {
            // Go straight to the default module
            $to = '/t/' . rawurlencode($slug) . '/apps/' . rawurlencode($def);
        } else {
            // Fallback: plain tenant dashboard
            $to = '/t/' . rawurlencode($slug) . '/dashboard';
        }
    ...
    if ($tail === 'dashboard' && $method === 'GET') {
        (new \App\Controllers\Tenant\DashboardController())->index(['slug' => $slug]);
        exit;
    }
    ...
    if ($tail === 'settings') {
        if     ($method === 'GET')  {
            (new \App\Controllers\Tenant\SettingsController())->index(['slug' => $slug]);
            exit;
        } elseif ($method === 'POST') {
            (new \App\Controllers\Tenant\SettingsController())->update(['slug' => $slug]);
            exit;
        }
    }
    ...
    if ($tail === '_dbcheck' && $method === 'GET') {
        ...
        $dbName = 'n/a';
        try {
            $dbName = $tenantPdo->query('SELECT DATABASE()')->fetchColumn() ?: 'n/a';
        } catch (\Throwable $e) {}
