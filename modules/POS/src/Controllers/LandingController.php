...
{
    /**
     * GET /apps/pos  — POS Apps launcher
     * Renders a full HTML page directly (NO shell).
     */
    public function home(?array $ctx = null): void
    {
        // Normalised ctx from BaseController
        $c = $this->ctx($ctx ?? []);
        ...
        // Single landing view file
        $view = $dir . '/Views/landing/index.php';

        try {
            if (!is_file($view)) {
                throw new \RuntimeException("POS landing view missing: {$view}");
            }
