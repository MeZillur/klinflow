...
/* POS — Apps (SVG sprite edition, BizFlow-style layout) */
$h = fn($s) => htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');

$brand = $brandColor ?? '#228B22';
$tint  = '#ffffff'; // clear white background

// Org info (from controller/session)
...
/**
 * IMPORTANT:
 *  - For POS, module base is /apps/pos (tenant routing adds /t/{slug} when needed)
 */
$base      = rtrim((string)($base ?? '/apps/pos'), '/');
$logoutUrl = '/tenant/logout';

/* -------------------------------------------------------------
 * Org logo resolution (POS)
 * 1) Try to build a data: URL from:
 *    modules/POS/Assets/Brand/logo/org_{id}/logo.*
 *    modules/POS/Assets/Brand/logo/{id}/logo.*
 * 2) If file read fails, fall back to HTTP path
 * 3) Final <img> uses global /assets/brand/logo.png as last fallback
 * ----------------------------------------------------------- */
...
if ($orgId > 0) {
    ...
    // __DIR__ = modules/POS/Views/landing
    // dirname(__DIR__, 2) = modules/POS
    $root = dirname(__DIR__, 2) . '/Assets';

    $dirs = [
        $root . '/Brand/logo/' . $orgKey,
        $root . '/Brand/logo/' . $orgId,
        $root . '/brand/logo/' . $orgKey,
        $root . '/brand/logo/' . $orgId,
    ];

    foreach ($dirs as $assetsFsBase) {
        if (!is_dir($assetsFsBase)) {
            continue;
        }
        foreach ($candidates as $file) {
            ...
            if ($raw !== false) {
                // Inline data: URL so avatar always previews, even if /modules not public
                $orgLogo = 'data:' . $mime . ';base64,' . base64_encode($raw);
            } else {
                // Fallback HTTP path; folder name must match case on disk
                $folder  = basename($assetsFsBase); // "7" or "org_7"
                $orgLogo = '/modules/POS/Assets/Brand/logo/' . $folder . '/' . $file;
            }
            break 2;
        }
    }
...
/* Apps (labels kept exactly; Dashboard first)
...
<!-- On-page SVG sprite (POS icons, same set as before) -->
...
<main class="max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-6">
  <div class="grid grid-cols-1 md:grid-cols-2 md:gap-10 items-start">
    ...
    <!-- LEFT: hero + org identity card + clock + calendar (desktop only) -->
    <section class="order-2 md:order-1 hidden md:block hero">
      ...
      <!-- Organisation block: logo + Name/Address/Phone/Email (same as BizFlow) -->
      ...
      <!-- Time + Calendar cards (BizFlow style) -->
