...
/**
 * MedFlow module shell (POS-style)
 */
...
$module_base = $module_base ?? ($slug ? "/t/{$slug}/apps/medflow" : '/apps/medflow');
$tenantDash  = $slug ? "/t/{$slug}/dashboard" : '/';

/* ---------- Robust sidenav autodetect ---------- */
$moduleSidenav = (isset($moduleSidenav) && is_string($moduleSidenav) && is_file($moduleSidenav)) ? $moduleSidenav : '';
if ($moduleSidenav === '') {
  // this file lives in modules/medflow/Views/shared/layouts/shell.php
  $medRoot = dirname(__DIR__, 3); // modules/medflow
  foreach ([
    $medRoot . '/Views/shared/partials/sidenav.php',
    $medRoot . '/src/Views/shared/partials/sidenav.php',
    $medRoot . '/medflow_sidenav.php',
  ] as $p) {
    if (is_file($p)) { $moduleSidenav = $p; break; }
  }
}
?>
<!doctype html>
...
<head>
  ...
  <title><?= $h($title ?? $orgName.' — MedFlow') ?></title>
...
<body class="bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
      x-data="{sidebar:false,dark:document.documentElement.classList.contains('dark'),menu:false}">
  ...
  <div class="w-full h-full lg:grid lg:grid-cols-[18rem_1fr]">
    ...
    <aside class="fixed z-40 inset-y-0 left-0 w-72 lg:static lg:w-auto bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transform transition-transform"
      ...
      <div class="h-[calc(100dvh-4rem)] overflow-y-auto">
        <?php
          if ($moduleSidenav && is_file($moduleSidenav)) {
            $module_base = $module_base; $org = $org;
    ...
    <div class="min-w-0 lg:min-w-[0]">
      <header class="h-16 sticky top-0 z-20 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-3 lg:px-4">
        <div class="flex items-center gap-2">
          <button class="lg:hidden p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700"
                  @click="sidebar=true" aria-label="Open menu">
            <i class="fa fa-bars"></i>
          </button>
          <div class="font-semibold truncate"><?= $h($orgName) ?></div>
          <span class="ml-2 text-xs rounded px-2 py-0.5 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">MedFlow</span>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="relative" @keydown.escape="menu=false">
            ...
            <div x-show="menu" x-cloak x-transition.origin.top.right
              ...
              <div class="px-4 py-3 text-sm border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="text-xs text-gray-500 truncate">@<?= $h($_SESSION['tenant_user']['username'] ?? '') ?></div>
              </div>
              <a href="<?= $h($tenantDash) ?>" class="flex items-center gap-2 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800 text-sm">
                <i class="fa-solid fa-gauge-high"></i> <span>Tenant Dashboard</span>
              </a>
