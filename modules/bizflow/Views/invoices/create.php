...
/**
 * BizFlow — Invoice Create (content-only view)
 *
 * Expects:
 *   $ctx, $module_base, $invoice_no, $customers, $payment_methods
 * Optional:
 *   $org
 */
$h = fn($s)=>htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');

$module_base = $module_base ?? ($ctx['module_base'] ?? '/apps/bizflow');
...
<div class="sr-shell">
  ...
  <div class="sr-header">
    <div class="sr-header-title">
      <h1>Invoice Entry</h1>
      <div class="hint">Fast B2B invoice • Category / Brand filter • Barcode • Keyboard shortcuts</div>
    </div>
    <div class="sr-header-actions">
      <a class="btn btn-muted" href="<?= $h($base) ?>/invoices">Invoices</a>
      <a class="btn btn-muted" href="<?= $h($base) ?>/invoices/drafts">Drafts</a>
      <a class="btn btn-muted" href="<?= $h($base) ?>/invoices/cancelled">Cancelled</a>
    </div>
  </div>
      ...
      <div class="sr-panel">
        <div class="sr-panel-inner">
          ...
          <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:8px;">
            <input id="productSearch" class="sr-input" placeholder="Type to search by name / code">
          ...
          <div class="sr-tiles-wrap">
            <div id="productTiles" style="display:grid;grid-template-columns:1fr;gap:8px;">
              <div class="sr-empty">Start typing to load items.</div>
            </div>
          </div>

          <div class="hint" style="margin-top:8px;">
            Tip: Choose <strong>Category</strong> or <strong>Brand</strong>, type an item keyword.
      ...
      <form id="invoiceForm" class="space-y-4">
        ...
        <div class="sr-panel">
          <div class="sr-panel-inner">
            <div class="sr-row">
              ...
              <div class="sr-col-3">
                <label class="block text-sm font-semibold mb-1" style="font-size:13px;font-weight:600;margin-bottom:4px;">Invoice Date</label>
                <input class="sr-input" type="date" name="invoice_date" value="<?= $h(date('Y-m-d')) ?>">
              </div>
              <div class="sr-col-3">
                ...
                <input class="sr-input" style="margin-top:8px;" name="customer_name" placeholder="(optional) invoice display name">
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="sr-panel">
          <div class="sr-lines-head">
            ...
            <div class="sr-text-right">Unit Price</div>
            <div class="sr-text-right">Qty</div>
            <div class="sr-text-right">Line Total</div>
            <div></div>
          </div>
            ...
            <div class="sr-panel">
              <div class="sr-panel-inner">
                ...
                <div style="display:flex;gap:12px;margin-top:16px;justify-content:flex-end;">
                  <a href="<?= $h($base) ?>/invoices" class="btn btn-danger">Cancel</a>
                  <button id="completeBtn" type="submit" class="btn btn-primary">Save & Pay</button>
                </div>
              </div>
            </div>
          </div>
...
const TAX_RATE = 0.05; // 5% VAT (adjust per bizflow if needed)
...
function render(){
  ...
  items.forEach((it,idx)=>{
    const line = (it.unit_price||it.sale_price||0) * (it.qty||0);
    sub += line;
    ...
    row.innerHTML = `
      <div>
        ...
        <div class="sr-line-sku">Code: ${it.code || it.sku || ""}</div>
      </div>

      <div class="sr-text-right">
        <input class="line-input sr-text-right" value="${it.unit_name||'pcs'}" readonly>
      </div>

      <div class="sr-text-right">
        <input class="line-input sr-text-right li-price" data-i="${idx}" type="number" step="0.01" value="${money(it.unit_price||it.sale_price||0)}">
      </div>
...
/* ================================================================
   PRODUCT SEARCH / CATEGORY / BRAND (BizFlow invoice)
   NOTE: implement /invoices/api/items in BizFlow backend
==================================================================*/
async function searchProductsByMode(q, mode){
  const p = new URLSearchParams({ q, mode });
  return unwrapList(await fetchJSON(`${BASE}/invoices/api/items?${p}`));
}
...
function addOrBump(prod, qty=1){
  ...
  if(ex){
    ex.qty += qty;
    if(!ex.unit_price) ex.unit_price = basePrice;
  }else{
    items.push({
      id:   prod.id,
...
async function loadTiles(){
  ...
  if(!rows.length){
    host.innerHTML = '<div class="sr-empty">No items found.</div>';
    return;
  }
...
/* ================================================================
   BARCODE ADD (BizFlow invoice)
   NOTE: implement exact_barcode filter in /invoices/api/items
==================================================================*/
$("#barcodeInput").addEventListener("keydown", async(e)=>{
  ...
  const res = await fetchJSON(
    `${BASE}/invoices/api/items?` + new URLSearchParams({q:code, exact_barcode:"1"})
  );
...
/* ================================================================
   NEW CUSTOMER QUICK ADD (BizFlow invoice)
   NOTE: implement /invoices/api/customers.create in BizFlow backend
==================================================================*/
$("#btnNewCustomer").onclick = async ()=>{
  const name = prompt("Customer name"); if(!name) return;
  const phone= prompt("Mobile");

  const res = await fetchJSON(`${BASE}/invoices/api/customers.create`, {
    method:"POST",
...
/* ================================================================
   PAYMENT MODAL (same as POS, reused)
==================================================================*/
...
function openPayModal(){
  ...
  payItems.value = items.map(it=>
    `${it.name} × ${it.qty} @ ${money(it.unit_price||it.sale_price||0)}`
  ).join("\n");
...
/* ================================================================
   FINAL SAVE (BizFlow invoice)
   NOTE: implement POST /invoices in BizFlow backend
==================================================================*/
async function performSave(){
  ...
  const payload={
    ...
    items: items.map(it=>({
      ...
      qty:        it.qty,
      unit_price: it.unit_price || it.sale_price || 0
    }))
  };
  ...
  if(r.ok){
    ...
    let id = j && (j.id || j.invoice_id);
    location.href = id ? `${BASE}/invoices/${id}` : `${BASE}/invoices`;
  }else{
    errSound();
    alert("Save failed!");
  }
