...
// Org info (already enriched via LandingController from identity.json)
...
/* -------------------------------------------------------------
 * Org logo resolution
 * 1) Try to build a data: URL from
 *    modules/bizflow/Assets/brand/logo/org_{id}/logo.*
 *    (works even if /modules is NOT web-accessible)
 * 2) If file read fails, fall back to HTTP path
 * 3) Final <img> uses /assets/brand/logo.png as last fallback
 * ----------------------------------------------------------- */

$orgLogo = ''; // will become either data: URL or /modules/... path
...
if ($orgId > 0) {
    ...
    // __DIR__ = modules/bizflow/Views/landing
    // dirname(__DIR__, 2) = modules/bizflow
    $assetsFsBase = dirname(__DIR__, 2) . '/Assets/brand/logo/' . $orgKey; // modules/bizflow/Assets/brand/logo/org_X

    foreach ($candidates as $file) {
        $fsPath = $assetsFsBase . '/' . $file;
        if (is_file($fsPath)) {
            // Try to read file and build data: URL (like Quotes print/PDF)
            ...
            if ($raw !== false) {
                // Inline data: URL → browser preview will always work
                $orgLogo = 'data:' . $mime . ';base64,' . base64_encode($raw);
            } else {
                // Fallback to HTTP path if for some reason file_get_contents fails
                // Folder names are all lowercase ("bizflow") to match the filesystem
...
<svg xmlns="http://www.w3.org/2000/svg" style="display:none">
  ...
  <symbol id="ico-hrm" viewBox="0 0 64 64"> <rect width="64" height="64" rx="14" fill="#EDE9FE"/> <circle cx="32" cy="24" r="8" fill="#4C1D95"/><circle cx="32" cy="22.5" r="5" fill="#F9FAFB"/><path d="M20 44c1.5-7 5.5-11 12-11s10.5 4 12 11v3H20v-3z"fill="#7C3AED"/><circle cx="18" cy="26" r="4.5" fill="#C4B5FD"/><circle cx="46" cy="26" r="4.5" fill="#C4B5FD"/><path d="M11 44c1.2-5.5 3.8-8.5 7.5-9.3M45.5 34.7C49.2 35.5 51.8 38.5 53 44"fill="none" stroke="#A855F7" stroke-width="3"stroke-linecap="round"/></symbol>

  <!-- Expenses (orange bill + down arrow) -->
  <symbol id="ico-expenses" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" fill="#FEF3C7"/><path d="M20 18h24v24l-3-2.5-3 2.5-3-2.5-3 2.5-3-2.5-3 2.5V18z"fill="#FDBA74" stroke="#F97316" stroke-width="2"stroke-linejoin="round"/><path d="M26 24h12M26 29h8"stroke="#7C2D12" stroke-width="3" stroke-linecap="round"/><circle cx="44" cy="42" r="8" fill="#F97316"/><path d="M44 38v6M40.5 41.5L44 45l3.5-3.5"stroke="#FEFCE8" stroke-width="2.5" stroke-linecap="round"stroke-linejoin="round"/></symbol>
  
  <!-- Settings icon -->
  <symbol id="ico-settings" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" fill="#F0FDF4"/><path d="M32 22a10 10 0 1 0 .001 20.001A10 10 0 0 0 32 22zm0-6 4 4h8v8l4 4-4 4v8h-8l-4 4-4-4h-8v-8l-4-4 4-4v-8h8l4-4z"fill="#166534"/></symbol>
</svg>
...
<script>
  ...
  // Home (landing) URL from PHP
  var homeUrl = <?= json_encode($base, JSON_UNESCAPED_SLASHES) ?>;
  ...
  // Keyboard shortcuts: ESC → landing, L → logout
  document.addEventListener('keydown', function(e){
    ...
    // ESC: first close drawer if open; otherwise go to landing page
    if (key === 'escape') {
