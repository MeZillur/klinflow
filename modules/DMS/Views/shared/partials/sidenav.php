...
and cross-module links (Dispatch/Forecast/Auto-PO) use tenant root /t/{slug}.
==============================================================================
*/
...
$tenantRoot  = $slug !== '' ? ('/t/'.rawurlencode($slug)) : '';
$brand       = $slug !== '' ? ucfirst($slug) : 'KlinFlow';

/* ────────────────────────────────────────────────────────────────────
 * SEGMENT 2: Menu model (module-local)
 * ────────────────────────────────────────────────────────────────── */
$menu = function (string $b) use ($tenantRoot): array {
  $b = rtrim($b, '/');
  return [
    [
    ...
    // RENAMED: Sales & Dispatch + added Dispatch Challans (points to /t/{slug}/dms/challan)
    [
      'key'=>'sales', 'icon'=>'fa-bag-shopping', 'label'=>'Sales & Dispatch',
      'children'=>[
        ...
        ['label'=>'Dispatch Challans',   'href'=>"$tenantRoot/dms/challan"],   // ← new
        ['label'=>'Payments',            'href'=>"$b/payments"],
        ['label'=>'Returns',             'href'=>"$b/returns"],
      ]
    ],

    // ADDED: Demand Forecasting + Auto Purchase Orders under Products & Purchases
    [
      ...
      'children'=>[
        ...
        ['label'=>'Demand Forecasting',  'href'=>"$tenantRoot/dms/forecast"],  // ← new
        ['label'=>'Auto Purchase Orders','href'=>"$tenantRoot/dms/auto-po"],   // ← new
      ]
    ],
  ...
  <div class="px-2 pt-4">
    ...
    <?php foreach ($sections as $sec): ?>
      ...
      <div class="mb-1 kf-group"
        ...
        <!-- Group body (NO LEFT BORDER ANYMORE) -->
        <div id="sub-<?= $h($groupId) ?>"
             class="kf-sub ml-2 pl-1 mt-1 <?= $open ? '' : 'hidden' ?>">
...
<!-- SEGMENT E: Minimal styles (brand + focus; borders removed) -->
<style>
  .kf-head { transition: background .2s; border: 0 !important; }
  .kf-sub  { border: 0 !important; }
