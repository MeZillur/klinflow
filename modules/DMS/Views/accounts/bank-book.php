...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between flex-wrap gap-3">
    ...
    <div class="flex items-center gap-2">
      <a href="?create=1" class="px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700">+ New Account</a>
    </div>
  </div>
    ...
    <table class="min-w-full text-[13px]">
      ...
      <tbody>
        ...
        <tr class="border-t hover:bg-gray-50 dark:hover:bg-gray-800/40">
          ...
          <td class="px-3 py-2 text-center">
            <a href="bank-accounts/<?= $h($r['id']) ?>/edit" class="text-emerald-600 hover:underline text-sm">Edit</a>
            <?php if (empty($r['is_master'])): ?>
              · <a href="bank-accounts/<?= $h($r['id']) ?>/make-master" class="text-gray-500 hover:text-emerald-700 text-sm">Make master</a>
            <?php endif; ?>
          </td>
        </tr>
        <?php endforeach; else: ?>
        <tr>
          <td colspan="7" class="px-3 py-10 text-center text-gray-500 dark:text-gray-400">
            ...
            <a href="?create=1" class="underline text-emerald-600">Create your first bank account</a>
          </td>
        </tr>
        <?php endif; ?>
      </tbody>
