<?php
/**
 * modules/DMS/Views/orders/print.php
 * Standalone printable Order view (hides app chrome).
 * Expects: $order, $items, $org (name,address,phone,email,logo_url), $module_base
 */
...
$discT  = (string)($order['discount_type'] ?? 'amount');
$discV  = (float)($order['discount_value'] ?? 0);
...
$subtotal = 0.0;
foreach ($items as $ln) {
    $q=(float)($ln['qty'] ?? $ln['quantity'] ?? 0);
    $p=(float)($ln['unit_price'] ?? $ln['price'] ?? $ln['rate'] ?? 0);
...
$auto = (isset($_GET['autoprint']) && $_GET['autoprint'] == '1');
?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title><?= h("Order {$no}") ?></title>
...
<div class="toolbar">
  <a class="btn" href="<?= h($base.'/orders') ?>">← Back</a>
  <button class="btn brand" onclick="window.print()">Print</button>
  <a class="btn" href="<?= h($base.'/orders/'.$id.'/edit') ?>">Edit</a>
</div>
      ...
      <div class="card">
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
          <div>
            ...
            <div class="val"><?= h($odate ?: '—') ?></div>
          </div>
          <div>
            <div class="label">Delivery Date</div>
            <div class="val"><?= h($ddate ?: '—') ?></div>
          </div>
        </div>
      </div>
    </div>
