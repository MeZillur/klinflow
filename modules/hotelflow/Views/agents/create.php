...
<div class="max-w-[700px] mx-auto space-y-4">
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-extrabold">Add Agent</h1>
    <a href="<?= $h($base) ?>/agents" class="px-3 py-2 rounded-lg border">Back</a>
  </div>
  <form method="post" action="<?= $h($base) ?>/agents" class="grid grid-cols-1 gap-3">
    <div>
      <label class="text-sm text-slate-600">Name</label>
      <input name="name" required class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-300">
    </div>
    <div class="flex justify-end gap-2">
      <a href="<?= $h($base) ?>/agents" class="px-4 py-2 rounded-lg border">Cancel</a>
