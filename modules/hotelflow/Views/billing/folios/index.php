...
<div class="flex items-center justify-between mb-3">
  <h1 class="text-2xl font-extrabold">Folios</h1>
</div>
...
<div class="overflow-auto rounded-xl border border-slate-200">
  <table class="min-w-[900px] w-full text-sm">
    ...
    <tbody>
      <?php if (!$rows): ?>
      <tr><td colspan="6" class="px-3 py-6 text-center text-slate-500">No folios found.</td></tr>
      <?php endif; foreach ($rows as $r): ?>
      <tr class="border-t">
        <td class="px-3 py-2 font-medium"><?= $h((string)$r['folio_no']) ?></td>
        <td class="px-3 py-2"><?= $h((string)($r['guest_name'] ?? '')) ?></td>
        ...
        <td class="px-3 py-2 text-right">
          <a class="px-3 py-1.5 rounded-lg border hover:bg-slate-50" href="<?= $h($base) ?>/billing/folios/<?= (int)$r['id'] ?>">Open</a>
        </td>
      </tr>
      <?php endforeach; ?>
    </tbody>
