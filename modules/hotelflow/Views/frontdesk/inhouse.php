...
<div class="max-w-[1100px] mx-auto space-y-6"
     x-data="hfInhouse(<?= json_encode(['base'=>$base,'seed'=>$rows,'date'=>$date]) ?>)"
     x-init="boot()">
  ...
  <div class="flex flex-col sm:flex-row sm:items-end gap-3 justify-between">
    <div>
      ...
      <p class="text-slate-500 text-sm">Guests in-house on <span x-text="date"></span>.</p>
    </div>
  ...
  <!-- Tabs -->
  <?php $tab='inhouse'; include __DIR__ . '/_tabs.php'; ?>
    ...
    <table class="min-w-full text-sm">
      <thead class="bg-slate-50">
        <tr class="text-left">
          ...
          <th class="px-3 py-2">Planned Out</th>
      ...
      <tbody>
        <?php if (!$rows): ?>
          <tr><td colspan="8" class="px-3 py-6 text-center text-slate-500">No in-house guests for <?= $h($date) ?>.</td></tr>
        <?php else: foreach ($rows as $r): ?>
          <tr class="border-t border-slate-200">
            <td class="px-3 py-2 font-medium">#<?= $h((string)($r['id'] ?? 0)) ?></td>
            ...
            <td class="px-3 py-2"><?= $h((string)($r['check_in_at'] ?? '')) ?></td>
            <td class="px-3 py-2"><?= $h((string)($r['planned_check_out'] ?? ($r['actual_check_out'] ?? ''))) ?></td>
            <td class="px-3 py-2">
              <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-emerald-50 text-emerald-700 border border-emerald-200">
                <i class="fa-solid fa-bed"></i> In-house
              </span>
            </td>
            <td class="px-3 py-2 text-right">
              <a href="<?= $h($base) ?>/reservations/<?= (int)($r['reservation_id'] ?? 0) ?>" class="text-emerald-700 hover:underline">Open</a>
            </td>
          </tr>
        ...
        <template x-for="row in list" :key="row.id">
          <tr class="border-t border-slate-200">
            <td class="px-3 py-2 font-medium">#<span x-text="row.id||''"></span></td>
            ...
            <td class="px-3 py-2" x-text="row.check_in_at || ''"></td>
            <td class="px-3 py-2" x-text="row.planned_check_out || row.actual_check_out || ''"></td>
            <td class="px-3 py-2">
              <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-emerald-50 text-emerald-700 border border-emerald-200">
                <i class="fa-solid fa-bed"></i> In-house
              </span>
            ...
            <td class="px-3 py-2 text-right">
              <a :href="base+'/reservations/'+(row.reservation_id||'')" class="text-emerald-700 hover:underline">Open</a>
            </td>
          </tr>
        </template>
      </tbody>
...
function hfInhouse(cfg){
  return {
    ...
    async pull(){
      ...
      try{
        const url = this.base + '/api/inhouse?date=' + encodeURIComponent(this.date);
